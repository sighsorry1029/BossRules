...
internal static class BossStonePerPlayerRuntime
{
    ...
    internal static bool TryHandleUseItem(ItemStand itemStand, Humanoid user, ItemDrop.ItemData item, out bool result)
    {
        ...
        if (!TryResolveBossStoneHandle(itemStand, out _) ||
    ...
    {
        result = false;
        if (!TryResolveBossStoneHandle(itemStand, out string playerKey))
        {
            return false;
        }

        result = Player.m_localPlayer != null && Player.m_localPlayer.HaveUniqueKey(playerKey);
        return true;
    }
    ...
    internal static bool TryOverrideUpdateVisual(ItemStand itemStand)
    {
        ...
        string itemName = Player.m_localPlayer != null && Player.m_localPlayer.HaveUniqueKey(playerKey)
    ...
    private static bool TryResolveBossStoneHandle(ItemStand? itemStand, out string playerKey)
    {
        playerKey = "";
        if (!BossRulesConfig.IsPerPlayerBossStonesEnabled() || itemStand == null || itemStand.m_guardianPower == null)
        {
            return false;
        }

        string guardianPowerName = itemStand.m_guardianPower.name?.Trim() ?? "";
        BossStoneItemStandRuntimeState state = itemStand.GetComponent<BossStoneItemStandRuntimeState>() ??
                                               itemStand.gameObject.AddComponent<BossStoneItemStandRuntimeState>();
        if (state.Resolved &&
